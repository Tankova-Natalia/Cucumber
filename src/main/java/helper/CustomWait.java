...
public class CustomWait {
    ...
    /**
     * Ждет появления заданного элемента. Если время ожидания превышает timeout секунд, тест не проходит.
     * @author Наталья Танькова
    ...
    public void untilPresenceOfElement(String selector) {
        ...
        while (driver.findElements(By.xpath(selector)).size()==0){
            ...
            if ((System.currentTimeMillis()-start)/1000 > timeout)
                Assert.fail("Время ожидания присутствия элемента " + selector + " вышло");
        }
        driver.manage().timeouts().implicitlyWait(timeout, TimeUnit.SECONDS);
    }
    /**
     * Ждет исчезновения заданного элемента. Если время ожидания превышает timeout секунд, тест не проходит.
     * @author Наталья Танькова
     * @param selector селектор элемента
     */
